...
```sh
make docker-build docker-push IMG=<some-registry>/remix:tag
```
...
```sh
make deploy IMG=<some-registry>/remix:tag
```
...
```sh
make build-installer IMG=<some-registry>/remix:tag
```
...
```sh
kubectl apply -f https://raw.githubusercontent.com/<org>/remix/<tag or branch>/dist/install.yaml
```
